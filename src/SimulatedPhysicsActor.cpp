...
SimulatedPhysicsActor::SimulatedPhysicsActor(PhysicsSimulator* simulator,
                                             btScalar          mass) {
	// Physics setup
	...
	btRigidBody::btRigidBodyConstructionInfo constructionInfo(
	  mass, motionState, s_defaultCollisionShape.get(), inertia);
...
void SimulatedPhysicsActor::translate(float longitude,
                                      float latitude,
                                      float altitude) {
...
void SimulatedPhysicsActor::rotate(float roll, float pitch, float yaw) {
	btQuaternion originalOrientation(
	  m_orientation.x, m_orientation.y, m_orientation.z, m_orientation.w);

	Actor::rotate(roll, pitch, yaw);

	btQuaternion newOrientation(
	  m_orientation.x, m_orientation.y, m_orientation.z, m_orientation.w);
